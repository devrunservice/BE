...
//        errorResponse.setMessage(errorMessage);
	...
	protected String getResultMessage(final Iterator<ConstraintViolation<?>> violationIterator) {
		...
		while (violationIterator.hasNext() == true) {
			...
			resultMessageBuilder.append("['").append(getPopertyName(constraintViolation.getPropertyPath().toString())) // 유효성
																														// 검사가
																														// 실패한
																														// 속성
					.append("' is '").append(constraintViolation.getInvalidValue()) // 유효하지 않은 값
					.append("'. ").append(constraintViolation.getMessage()) // 유효성 검사 실패 시 메시지
					.append("]");
	...
	protected ResponseEntity<Object> handleHttpMessageNotReadable(HttpMessageNotReadableException ex,
			HttpHeaders headers, HttpStatus status, WebRequest request) {
		Map<String, String> responemap = new HashMap<>();
		responemap.put("message",
				"Can't Read HTTP Request, Check Your Request data type. ex json, xml, text, Number, Array, Object etc. -By DevRun");
