...
public class YouTubeUploader {
    ...
    public VideoDto uploadVideo(VideoDto videoDto, HttpServletResponse httpServletResponse, String accessToken) throws IOException {
        try {
            ...
            // 업로드 명령에는 다음이 포함됩니다: 파일이 성공적으로 업로드된 후 반환되는 정보, 업로드된 비디오와 연결되는 메타데이터, 비디오 파일 자체.
            ...
            /*
             * 직접 미디어 업로드가 활성화되었는지 여부를 설정합니다. True = 전체 미디어 콘텐츠를 한 번의 요청으로 업로드합니다.
             * False (기본값) = 데이터 청크로 업로드하기 위한 재개 가능한 미디어 업로드 프로토콜을 사용합니다.
             */
